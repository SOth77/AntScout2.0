...
import de.fhwedel.antscout.map.Node
...
object AntMap extends Logger {
  ...
  def computeAntWays(wayData: Set[AntWayData]) {
    ...
    val ss = wayData.toList.sorted
    //wayData.foreach { x => info("wayData: %s".format(x.nodes.toString)) }
    //ss.foreach { x => info("ss: %s".format(x.nodes.toString)) }

    //_ways = (1 to wayData.size).zip(wayData).map {
    _ways = (1 to ss.size).zip(ss).map {
   ...
   */
  def calculateShortestPaths(adjacencyMatrix: Map[Node, Map[Node, Double]], predecessorMatrix: Map[Node, Map[Node, Option[Node]]]): (Map[Node, Map[Node, Double]], Map[Node, Map[Node, Option[Node]]]) = {
    ...
    val (time, (distanceMatrix, intermediateMatrix)) = TimeHelpers.calcTime {
      ...
      val distanceMatrix = new mutable.HashMap[Node, mutable.Map[Node, Double] with mutable.SynchronizedMap[Node, Double]] with mutable.SynchronizedMap[Node, mutable.Map[Node, Double] with mutable.SynchronizedMap[Node, Double]]
      ...
      val intermediateMatrix = new mutable.HashMap[Node, mutable.Map[Node, Option[Node]] with mutable.SynchronizedMap[Node, Option[Node]]] with mutable.SynchronizedMap[Node, mutable.Map[Node, Option[Node]] with mutable.SynchronizedMap[Node, Option[Node]]]
      predecessorMatrix.par.foreach {
        case (source, predecessors) =>
          intermediateMatrix += source -> new mutable.HashMap[Node, Option[Node]] with mutable.SynchronizedMap[Node, Option[Node]]
  ...
  def predecessorMatrix = {
    ...
    nodes.map { source =>
      source -> nodes.map { destination =>
        destination -> {
          if (destination != source && distanceMatrix(source)(destination) < Double.PositiveInfinity)
            Some(source)
          else
            None
        }
